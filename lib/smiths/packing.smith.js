...
PackingSmith.prototype = {
  ...
  'addMatrix': function (name, matrix) {
    var item = {
          'width': matrix[0],
          'height': matrix[1]
        };
  ...
  'addItemObject': function (itemObj) {
    var keys = Object.getOwnPropertyNames(itemObj),
        that = this;
    keys.forEach(function (name) {
      that.addItem(name, itemObj[name]);
    });
  },
  'addMatrixObject': function (itemObj) {
    var keys = Object.getOwnPropertyNames(itemObj),
        that = this;
    keys.forEach(function (name) {
      that.addMatrix(name, itemObj[name]);
    });
  },
  ...
  'getItemMap': function () {
    // Iterate over the packed items and compile a map
    var packedItems = this.packedItems,
        retObj = {};
  ...
  /**
   * @param {Object} options Options to export via
   * @param {Boolean} [options.normalize=true]
   */
  'exportRaw': function (options) {
    // Fallback options
    options = options || {};

    // If specified, normalize the coordinates to 0, 0
    var normalizeCoordinates = options.normalize;
    if (normalizeCoordinates === undefined) { normalizeCoordinates = true; }
  ...
  'export': function (options) {
    ...
    var items = this.exportItems(options),
        stats = this.getStats();
